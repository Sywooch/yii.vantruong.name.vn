...
function get_link_edit($id = 0, $o = array()){
	...
	$html = $edit ? cu([__RCONTROLLER__ .'/edit','id'=>$id]+$param+get_params([],['id'])) : '#'; 
	return $html;
}
...
function Ad_edit_show_text_field($v=[],$o=[]){
	...
	$html= '<div class="form-group"><label class="col-sm-12 control-label">'.$label.'</label><div class="col-sm-12"><'.$input;
	if(!empty($attrs)){
		foreach ($attrs as $k=>$v){
			$html .= " $k=\"$v\"";
		}
	}
	$html .= ' type="'.$input_type.'" name="'.$field_name.'" class="form-control input-sm '.$class.'" placeholder="'.$placeholder.'" '.($input == 'input' ? 'value="'.uh($value).'" /' : "").'>'.($input == 'input' ? '' : uh($value)."</$input>").'</div>';
...
function genCustomerCode($regex = [],$i=0){
	...
	if(isset($regex['auto_code']) && $regex['auto_code'] == 'on'){
		...
		if(!$sort_asc){
			...
			$sql .= ") from customers where sid=".__SID__;
		...
		if((new \yii\db\Query())->from('{{%customers}}')->where(['code'=>$code,'sid'=>__SID__])->count(1) >0){
