...
class Mytemplete extends \yii\db\ActiveRecord
{
    ...
    {
        return Templete::tableName();
    }
    public static function tableToShop()
    {
    	return Templete::tableToShop();
    }
    ...
    public static function getList($o = []){
    	...
    	->innerJoin(['b'=>self::tableToShop()],"a.id=b.temp_id")
    	->andWhere(['b.sid'=>__SID__])
    	->andWhere(['>','a.state',-2]);
    	...
    	$query->select(['a.*','tState'=>'b.state','tLang'=>'b.lang'])
    	->orderBy($order_by)
    	->offset($offset)
    	->limit($limit);
    	$l = $query->asArray()->all();
