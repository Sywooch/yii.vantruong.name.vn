...
<table class="table table-bordered vmiddle table-hover">
  ...
  echo afShowThread(array(
	    ...
	    array(
  		...
  		'class'=>'center cactive','order'=>true,'field'=>'a.position'
  				],
  		[
...
echo '<tr class="tr_item_'.$v['id'].'">
		...
		<td class="center"><a href="'.afGetEditLink($v['id']).'">'.($v['code']).'</a></td>
  		<td class="center nowrap"><a href="'.afGetEditLink($v['id']).'">'.($v['short_name']).'</a></td> 
        <td class="nowrap"><a href="'.afGetEditLink($v['id']).'">'.uh($v['name']).'</a></td>
  		<td class="center nowrap">'.$v['localName'].'</td>
  		<td class="">'.$v['address'].'</td> 
        <td class="nowrap center">'.$v['phone'].'</td>
		<td class="nowrap center">'.$v['email'].'</td>';
echo Ad_list_show_qtext_field($v,[
        		'field'=>'position',
        		'class'=>'number-format ',
        		'decimal'=>0,
        		'role'=>$role
