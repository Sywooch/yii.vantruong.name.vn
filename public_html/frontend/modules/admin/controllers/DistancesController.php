...
use Yii;
use app\modules\admin\models\Distances;
...
/**
 * Ad_moduleController implements the CRUD actions for Distances model.
 */
class DistancesController extends CrsController
{
    /**
     * @inheritdoc
     */
    ...
    {    	
    	if(Yii::$app->request->method == 'POST'){
    		...
    		$f['sid'] = __SID__;
    		//Yii::$app->db->createCommand()->insert(Distances::tableName(),$f)->execute();
    		//$id = Yii::$app->db->createCommand("select max(id) from ".Distances::tableName())->queryScalar();
    		if(isset($f['distance'])){
    			$f['distance'] = cprice($f['distance']);
    		}
    		foreach (explode(',', $f['title']) as $title){
    			if(trim($title) != "" && (new yii\db\Query())->from(Distances::tableName())->where(['title'=>trim($title),'sid'=>__SID__])->count(1) == 0){
    				$f['title'] = trim($title);
    				Yii::$app->db->createCommand()->insert(Distances::tableName(),$f)->execute();
    				$id = Yii::$app->db->createCommand("select max(id) from ".Distances::tableName())->queryScalar();
    				$$this->model->updateHighway($id);
    			}
    		}
    		$btn = post('btnSubmit');
    		$tab = post('currentTab');
    ...
    {    	
    	...
    	if(Yii::$app->request->method == 'POST'){
    		...
    		// update with lang    		
    		Yii::$app->db->createCommand()->update(Distances::tableName(),$f,$con)->execute();
    		$this->model->updateHighway($id);
    		$btn = post('btnSubmit');
    		$tab = post('currentTab');
    		btnClickReturn($btn,$id,$tab);
    ...
    {
        if (($model = Distances::findOne(['id' => $id,'sid'=>__SID__])) !== null) {
            return $model;
        } else {
