...
$root_path_x = dirname(dirname(dirname(dirname(dirname(dirname(__FILE__))))));
$config = $root_path_x . '/common/config/main-local.php';
include_once $root_path_x . '/common/functions/_function.php';
$config = include_once($config);
include_once($root_path_x.'/common/models/MiniApp.php');
include_once($root_path_x.'/vendor/yiisoft/yii2/web/FtpUpload.php');
$db = new \common\models\MiniApp($config['components']['db']);
$config['db']['connectionString'] = $db->connectDataBase(); 
defined('__SID__') or define('__SID__', $db->getSID())  ;
defined('__SITE_NAME__') or define('__SITE_NAME__', $db->getSCode())  ;
$server_information = $db->getHost();
$x = $db->_getConfigs('SITE_CONFIGS',__SID__);

//if(isset($x['other_setting']['host_media']) && ($x['other_setting']['host_media'] == 'on' || $x['other_setting']['host_media'] == 1)){
	define('USED_HOST_MEDIA', true);
//}else{
//	define('USED_HOST_MEDIA', false);
//}
if(USED_HOST_MEDIA && !empty($server_information)){
    define('CONNECTED_FTP_MEDIA',true);
    global $_FTP;
    $_FTP = new \yii\web\FtpUpload($server_information);   
    //var_dump($_FTP);exit;
    define('FTP_ROOT_PATH',dString($server_information['root_directory']));
    $baseUrl = dString($server_information['web_address']).'/'.__SITE_NAME__.'/';
    $baseDir = dString($server_information['root_directory']).'/'.__SITE_NAME__.'/';
}else{
	$s = $_SERVER;
	...
	$a = parse_url($url);
	$baseUrl = $protocol . '://' . $host . $port .'/medias/'. __SITE_NAME__ .'/'; 
    define('CONNECTED_FTP_MEDIA',false);
