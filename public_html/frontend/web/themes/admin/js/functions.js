...
function load_datetimepicker(){
	jQuery('.ajax-datetimepicker,ajax-timepicker,.ajax-datepicker,.datetimepicker,.datepicker,timepicker').each(function(i,e){
		var $e = jQuery(e);
		if($e.attr('data-loaded') == undefined){
			$format = $e.attr('data-format') ? $e.attr('data-format') : 'DD/MM/YYYY HH:mm:ss';
			$e.datetimepicker({
  ...
  for (var selector in chosen_config) {
    if(jQuery(selector).length>0){
		...
		jQuery('select.ajax-chosen-select-ajax').each(function(index,element){
			if(jQuery(element).attr('data-loaded') == undefined){
...
function validate_form_before_submit($t){
	...
	if($submit){
		...
		jQuery('.required').each(function(i,e){			
			...
			$minlength = parseInt($input.attr('data-min'));
...
function submitForm($t){
    ...
    if(parseInt($role) == 5){
       ...
       window.location = $cfg.cBaseUrl + '/' + $cfg.controller_text + $cfg.returnUrl;
    	...
    	jQuery('input.btnSubmit').val($role);$submit = validate_form_before_submit($t);
...
function show_left_small_loading($action){
	...
	case 'hide':
		...
		window.setTimeout(function() {jQuery(".alert-success").addClass('hide'); }, 1000); 
		break;
	case 'error':
		jQuery(".alert-success").removeClass('loading').addClass('error alert-danger alert-dismissible') ;
		window.setTimeout(function() {jQuery(".alert-success").addClass('hide'); }, 1500);  
		break;	
	}
...
function get_decimal_number($t){		 
	jQuery.ajax({
	      ...
	      success: function (data) {	    	  
	    	  jQuery($t).attr('data-decimal',data);
	    	  if(jQuery($t).attr('data-target')){
	    		jQuery(jQuery($t).attr('data-target')).val(data) 
	    	  }
	    	  if(jQuery($t).attr('data-target-input')){
	    		jQuery(jQuery($t).attr('data-target-input')).attr('data-decimal',data).attr('data-currency',jQuery($t).val()).number(true,data) 
	    	  }
...
function quick_search_station_for_add_supplier($t){
	...
	jQuery.ajax({
	      ...
	      datatype: 'json',
		  url: $cfg.adminUrl  + '/ajax',						 		 
	      data: {action:'quick_search_station_for_add_supplier','val':$this.val()},
	      beforeSend:function(){},
	      success: function (data) {
	    	  $d = JSON.parse(data);
...
function quick_search_tour_service($t){
	...
	var $data = getAttributes($this);
	// 
	$data['selected'] = jQuery("input.selected_value_"+$type_id+'_'+$data['day']+'_'+$data['time']).map(
	        function () {return this.value;}).get().join(",");
	$data['place_id'] = jQuery('.input-quick-search-local').val();
	$data['total_pax'] = jQuery('#input-tour-sokhach').val();
	$data['nationality'] = jQuery('#inputNationality').val();
...
function generateSitemap($t){
	$freq = jQuery('#sitemap_freq').val();
	$last_mod = jQuery('#sitemap_last_mod').val();
	$priority = jQuery('.sitemap_priority:checked').val();
	
	jQuery.ajax({
	      type: 'post',
	      datatype: 'json',
		  url: $cfg.adminUrl  + '/ajax',						 		 
	      data: {action:'generateSitemap',freq:$freq,priority:$priority,lastmod:$last_mod},
	      beforeSend:function(){},
	      success: function (data) {
	    	  $d = JSON.parse(data);
	    	    jQuery('#inpuSitemap').val($d.html);
	    	   // jQuery('.input_codemirror').each(function(i, el){
	    	    var myInstance = jQuery('#inpuSitemap').data('CodeMirrorInstance');
...
function reloadAutoPlayFunction(){
