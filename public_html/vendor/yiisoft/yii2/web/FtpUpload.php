...
function list_all_files($recursive = false, $path = false){
    ...
    $buff = ftp_rawlist($this->link_id, $path);
    /// view($buff);
    /// view(ftp_nlist ($this->link_id, $path));
	$find = 'public_html';
    ...
    if(count($buff)>0){
        foreach($buff as $result){
            ...
            if( $type == 'dir'){
                // recursively call the function if this file is a folder
				$flist[] = array('type'=>$type, 'path'=>$path, 'name'=>$r['zfile_name']);
...
function upload_files($post,$o = array()){
	...
	foreach($post['name'] as $k=>$v){
		...
		if($post['error'][$k] == 0){
			...
			if(!$error){
				...
				$folder = '/'.__SITE_NAME__.'/'.$folder_save;	$folder = str_replace('//','/',$folder);
...
function upload_files_no_ftp($post,$o = array()){
	$time = time();	
	$year = date("Y",$time); $month = date("m",$time);	
	$rename = isset($o['rename']) && $o['rename'] == false ? false : true;
	$replace = isset($o['replace']) && $o['replace'] == true ? true : false;
...
function multi_upload_images($folder_name,$post,$type="",$time=0,$re=0,$scale=0,$w=0,$h=0){
	...
	//var_dump($this->rand_str());

	if(!empty($post) && count($post['name'])>0)	{	

	//var_dump($post);

	for($iv=0;$iv<count($post['name']);$iv++){

		if($post['error'][$iv]>0){ exit();break;return;}

		

		$filename_img = $post['name'][$iv];

		$filesize_img = $post['size'][$iv];

		$filetype_img = $post['type'][$iv];

		$filetemp_img = $post['tmp_name'][$iv];		

		///var_dump($filename_img);

		switch($filetype_img){

			case 'image/jpeg':$fd="images"; break;

			case 'image/jpg':$fd="images"; break;

			case 'image/png':$fd="images"; break;

			case 'image/gif':$fd="images"; break;

			case 'image/bmp':$fd="images"; break;

			case 'image/x-icon':$fd="images"; break;

			

			default:$fd = "";return false; break;

			

		}

		$folder = '/'.$folder_name.'/'.$fd.'/'.$type.'/'.$year.'/'.$month;

				

		//var_dump($_FILES[$post]);
		...
		//var_dump(count($list_name));

		$filename_save = md5($time.$this->rand_str()).".".$ftype;

		$filename_save=str_replace(" ","_",$filename_save);

		$list[$iv+1][0]=$filename_save;

		for($ll = 0;$ll<count($list_name);$ll++){

			$list[$iv+1][$ll+1]=$list_name[$ll];
		...
		$tmpname  = $filetemp_img; 

		$filetemp_img1 = $filetemp_img;

		$filetemp_img2 = $filetemp_img;

		$image = new SimpleImage();	

		$image->load($filetemp_img);	
		...
		if(!file_exists($dm->physical_add.$folder.'/origin') || !is_dir($dm->physical_add.$folder.'/origin')){

				$this->MkDir_($dm->physical_add.$folder.'/origin');

		}

		$this->put($dm->physical_add.$folder.'/'.$filename_save, $filetemp_img);
 
	}

	}

		$list[0][0] = $dm->site_add.$folder;//return http://1.img.....

		$list[0][1] = $dm->physical_add.$folder;//return /domain/img.muasam.vn/public_html/...
	$this->close(); 
	return $list;	
	exit ();
}
